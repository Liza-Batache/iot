...
const db = require('../../db/connection'); // Chemin relatif pour accéder à connection.js
...
router.post('/', (req, res) => {
    ...
    if (!name || !ip_address) {
        return res.status(400).send('Les champs name et ip_address sont obligatoires.');
    }

    const query = 'INSERT INTO devices (name, ip_address) VALUES (?, ?)';
    db.query(query, [name, ip_address], (err) => {
        if (err) {
...
// Route pour récupérer les appareils d'un utilisateur spécifique
